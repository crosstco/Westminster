...
if (Meteor.isServer) {
  ...
  Meteor.methods({
    ...
    checkduplicatedActivity: function(activity){
      check(activity, {
        title: String,
        description: String,
        brainTargets: [String],
     ...
     var duplicate = Activities.find({title:"activity.title"}).count();
      if(duplicated != 0){
        throw new Meteor.Error("invalid-activity","duplicated name for the activity");
        return(false);
      }
